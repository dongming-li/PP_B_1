...
/*++
  ...
  Routine Description:
  	...
  	Creates a new/mounts an existing virtual file system driver. The API is complex so
	probably best to see example source code file on how to invoke the API to do various
	tasks.
	...
	otherwise it will not be formatted

	Example code can be found in the file example.c
  ...
  Return Value:
    	...
    	NTSTATUS value indicating success/failure
...
/*++
  ...
  Routine Description:
 	...
 	Internal routine handles setup and initialization of the VFS
...

{
	...
	//
	// Create device object and symbolic links
	//
	...
	if (NT_SUCCESS(status)) {
		...
		DbgPrint("[VFS] Failed to Create Device Object for Driver");
		return status;
	}

	//
	// Setup VFS_DEVICE_EXTENSION Structure
	//
	...
	//
	// Save attributes of memory mapped file
	//
	...
	//
	// Copy attributes into VFS Extension
	//
	...
	//
	// Calculate DISK_GEOMETRY Structure for VFS
	//
	...
	//
	// Format the drive as specified filesystem if necessary
	//
	...
	switch(VFSExtension->FileSystemType) {
		...
		default:
			DbgPrint("[VFS] Failure Unrecognized Filesystem Type Specified");
			return STATUS_UNSUCCESSFUL;
	}	
	...
	//
	// Return address of VFS object/handle to caller 
	//
...

{
	...
	//
	// Need to make sure we properly close and unmap the sections
